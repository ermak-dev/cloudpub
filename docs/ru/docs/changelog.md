...
## 3.0.2 (24 декабря 2025)

### Новые возможности

 - Добавлена сборка клиента для Android на архитектуре aarch64
 - Более удобная навигация, запуск/остановка публикации из формы
 - Добавлена возможность поиска публикаций по уникальному идентификатору
 - Платные пользователи больше не требуют подтверждения телефона

### Исправлены ошибки

 - Исправлены ошибки c подтверждением телефона
 - Улучшена производительность за счет Redis кэширования

## 3.0.1 (03 декабря 2025)

### Новые возможности

 - **Протокол PROXY v2 для TCP** - передача информации о клиенте для TCP протокола

### Исправлены ошибки

 - Неправильные ссылки на документацию
 - Неверно работает смена сервера в ONPREM версии

## 3.0.0 (1 декабря 2025)

### Новые возможности

 - **Бастион для RDP и VNC** - централизованный доступ к удаленным рабочим столам с записью всех действий пользователя
 - **Запись RDP сеансов** - автоматическое сохранение видео всех действий в формате MWRM с конвертацией в MP4
 - **Двухфакторная аутентификация (2FA)** - защита аккаунта с помощью TOTP (Google Authenticator, Яндекс Ключ) и резервных кодов
 - **Аудит действий** - детальное логирование всех операций пользователей (вход, создание, изменение, удаление, подключения)
 - **Группы пользователей** - создание команд с общим доступом к публикациям и гибкой системой прав
 - **Учетные данные** - централизованное хранение паролей и ключей с шифрованием для подключения к RDP/VNC серверам
 - **Вход по имени пользователя** - возможность входа как по email, так и по имени пользователя
 - **Валидация email и телефона** - подтверждение email и номера телефона
 - **Триграммный поиск** - полнотекстовый поиск с морфологией в логах и аудите
 - **Пользовательские страницы ошибок** - возможность настройки собственных страниц ошибок (On-Premise)

### Улучшения

 - Обновлен интерфейс с русскими метками и улучшенной локализацией
 - Улучшена обработка ошибок с более понятными сообщениями на русском языке
 - Переход на localStorage вместо cookies для хранения настроек
 - Оптимизирована архитектура API с переходом на новые endpoints
 - Улучшена производительность базы данных с частичными индексами
 - Добавлены sourcemaps для упрощения отладки
 - Обновлены зависимости до последних версий
 - Улучшена документация с добавлением разделов про 2FA, аудит и бастион

### Исправлены ошибки

 - Зарезервированные субдомены rdp, ssh, vnc теперь нельзя использовать
 - Исправлена обработка ошибок SMTP при отправке писем
 - Исправлено восстановление lease при таймауте
 - Траффик лимит теперь применяется только для пользователей без лицензии
 - Исправлены ошибки при редактировании профиля

## 2.4.5 (28 сентября 2025)

### Новые возможности

 - Повтор HTTP запроса из инспектора
 - GUI приложение запускается без разворачивания окна

### Исправлены ошибки

 - При закрытии GUI приложения на Windows в трее остается иконка
 - Убраны полосы прокрутки при малом размере окна
 - Ошибка при публикации 1С если в пути есть кириллица
 - Ломается верстка если имя или E-mail слишком длинные
 - Утилита clo для Mac и Windows теперь не зависит от системных библиотек
...
### Исправлены ошибки
 ...
 - Неверный статус остановленых публикаций при перезапуске приложения
